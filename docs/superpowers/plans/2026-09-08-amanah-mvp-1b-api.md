...
invitesRouter.post("/invites/:token/accept", requireAuth, async (req, res) => {
  ...
  try {
    const circleId = await withAdminTxn(async (q) => {
      ...
      // upsert / un-remove membership
      await q.query(
        `insert into public.circle_members (circle_id, user_id, role, is_family_member, invited_by)
         values ($1,$2,$3,$4, (select invited_by from public.invites where token = $5))
         on conflict (circle_id, user_id) where removed_at is null
         do update set role = excluded.role, is_family_member = excluded.is_family_member`,
        [inv.circle_id, claims.sub, inv.role, inv.is_family_member, req.params.token],
      ).catch(async (e) => {
        // conflict target is a partial index; fall back to explicit un-remove
        if (String(e.message).includes("no unique or exclusion constraint")) {
          const existing = await q.query(
            `select id from public.circle_members
             where circle_id=$1 and user_id=$2`, [inv.circle_id, claims.sub]);
          if (existing.rowCount) {
            await q.query(
              `update public.circle_members
               set removed_at = null, role = $3, is_family_member = $4
               where id = $1`, [existing.rows[0].id, null, inv.role, inv.is_family_member]);
          } else {
            await q.query(
              `insert into public.circle_members (circle_id,user_id,role,is_family_member,invited_by)
               values ($1,$2,$3,$4,$5)`,
...
test("accept with matching email joins the circle", async () => {
  const token = await seedInvite(fx.circleB1, "twoCircle@example.com", "caregiver");
  // twoCircle is already family in B1 -> this upgrades the membership
  const jwt = await mintJwt({ sub: fx.users.twoCircle, email: "twoCircle@example.com" });
...
test("double-accept: second call -> 410", async () => {
  const token = await seedInvite(fx.circleB1, "twoCircle@example.com");
  const jwt = await mintJwt({ sub: fx.users.twoCircle, email: "twoCircle@example.com" });
  const app = createApp();
